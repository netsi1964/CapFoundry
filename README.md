...
A call prints the result and, on stderr, what it cost:
...
`cfcm describe <capability>` prints the full contract without running anything.

Exit codes, so it behaves inside a script:
...
Exit `2` is the one worth wiring up, and it is deliberately not `1`. A search
that finds nothing is a normal answer rather than a failure, and a script that
treats the two the same will fight the design instead of using it.
