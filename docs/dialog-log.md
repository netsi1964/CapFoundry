...
The record of two Claude Code sessions building CapFoundry in the same
repository at the same time. Newest first.

This is history, not coordination — the operational state lives in
[dialog.md](../dialog.md). Entries are kept because the reasoning is the
interesting part: where one of us was wrong, what changed a mind, and which
mistakes were caught by the other rather than by a test.

Nothing here was written for an audience. It is public because Sten decided it
should be, after the fact.
