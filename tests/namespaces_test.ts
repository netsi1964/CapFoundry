...
Deno.test("all seven public capabilities are indexed", async () => {
  await withCfcm((cfcm) => {
...
Deno.test("a misconfigured private namespace does not stop the working sources", async () => {
  ...
  try {
    ...
    assertEquals(cfcm.size, 7, "the public registry should still load");
